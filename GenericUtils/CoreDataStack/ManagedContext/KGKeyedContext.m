...
#import "KGKeyedContext.h"
#import "CDDebugLog.h"

@interface KGKeyedContext ()
@property (nonatomic, strong) NSMutableDictionary *contextContainer;
@end

@implementation KGKeyedContext
...
+ (instancetype) sharedInstance{
    ...
    static KGKeyedContext *sharedInstance;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        sharedInstance = [[KGKeyedContext alloc] init];
    });
...
- (void)multiContextDidSave:(NSNotification*)notification {
    ...
    //Test If this is calling on main thread or not.
    [CDDebugLog message:@"contextDidSave execution queue :: %s",dispatch_queue_get_label(DISPATCH_CURRENT_QUEUE_LABEL)];
    //
