...
- (void) loadFromUrl:(NSURL*)fileUrl{
    ...
    if (self.mutableCollection) [CDDebugLog message:@"Successfully readed from fileSystem as %@",(self.isDictionary)?@"mutableDictionary":@"mutableArray"];
}
...
- (void) loadDefault:(NSURL*)fileUrl isDictionary:(BOOL)isDictionary{
    ...
    if (isDictionary) {
        ...
        if ([(NSMutableDictionary*)self.mutableCollection writeToURL:fileUrl atomically:YES]) {
            [CDDebugLog message:@"Successfully Written to fileSystem as mutableDictionary"];
        }
    }else{
        self.isDictionary = isDictionary;
        ...
        if ([(NSMutableArray*)self.mutableCollection writeToURL:fileUrl atomically:YES]) {
            [CDDebugLog message:@"Successfully Written to fileSystem as mutableArray"];
        }
    }
}
...
- (void)saveBackground{
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_HIGH, 0), ^{
        if ([self save]) {
            [CDDebugLog message:@"Successfully written to fileSystem in background"];
        }
    });
}
