...
#import "KGCoreObject.h"
#import "CDDebugLog.h"

@implementation KGCoreObject
@dynamic guid;
...
+ (NSUInteger)rows:(NSManagedObjectContext *)context{
    ...
    if (error != nil) {
        [CDDebugLog message:@"%@",[error debugDescription]];
    }
    return counts;
}
...
+ (KGCoreObject*) readBy:(NSManagedObjectID*)managedId context:(NSManagedObjectContext*)context{
    ...
    if (managedId.isTemporaryID == NO) {
        NSError *error;
        KGCoreObject *obj = (KGCoreObject*)[context existingObjectWithID:managedId error:&error];
        if (error != nil) {
            [CDDebugLog message:@"%@",[error debugDescription]];
        }
        return obj;
        ...
        [CDDebugLog message:@"%@ -> given ManagedId is a temporaryID, Please save to context, before further use.",NSStringFromClass([self class])];
    }
    return nil;
}
...
+ (NSFetchRequest*) createRequestWithEntityName{
    ...
    if (![encodeType isSubclassOfClass:[KGManagedObject class]]) {
        return nil;
    }
...
+ (KGCoreObject*) maxOrderedItem:(NSArray*)items{
    //
...
+ (void) write:(NSDictionary *)updateProperties whereGuid:(NSString *)guid context:(NSManagedObjectContext *)context{
    //
    KGCoreObject *item = [[self class] readByGuid:guid context:context];
    if (item) {
...
+ (void) write:(NSDictionary *)updateProperties whereManagedID:(NSManagedObjectID *)ID context:(NSManagedObjectContext *)context{
    //
    KGCoreObject *item = [[self class] readBy:ID context:context];
    if (item) {
        [item write:updateProperties];
    }
}
...
+ (void) insert:(NSArray*)items context:(NSManagedObjectContext *)context{
    ...
    for (NSDictionary *item in items) {
        ...
        [CDDebugLog message:@"Inserted :: %@", item];
    }
}
