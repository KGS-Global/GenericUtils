...
+ (instancetype)insertIntoContext:(NSManagedObjectContext *)context withProperties:(NSDictionary *)properties{
    ...
    KGManagedObject *dObject = [NSEntityDescription insertNewObjectForEntityForName:entity inManagedObjectContext:context];
    if (dObject) {
...
- (id) serializeValue:(id)value forKey:(NSString*)key{
    ...
    if ([value isKindOfClass:[KGManagedObject class]]) {
        
        result = [(KGManagedObject*)value serializeIntoInfo];
    }
