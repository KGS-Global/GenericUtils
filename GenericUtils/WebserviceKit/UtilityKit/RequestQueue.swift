...
@objcMembers
open class ContentDelegateImpl: KGObject, ContentDelegate {
...
@objcMembers
open class RequestQueueConfiguration: KGObject {
...
open class BaseRequestQueue: NSObject, RequestQueue {
    ...
    fileprivate func execute(){
        if !networkReachable{
            print("DOWNLOAD CANCEL DUE TO NETWORK REACHABILITY")
            return
        }
        if let tracker = requestQueue.dequeue() as? Tracker{
            if preCancelCheck(tracker.request){
...
open class SavableRequestQueue: BaseRequestQueue {
    ...
    public func deleteCache(_ onCompletion: (()->Void)?){
        guard let url = manager.storageLocation() else{
        ...
        manager = PropertyList(fileName: "\(identifier)_write_queue", directoryType: FileManager.SearchPathDirectory.documentDirectory, dictionary: true)
        orderIndex = -1
        if let completion = onCompletion{
            completion()
        }
