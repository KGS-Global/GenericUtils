    ...
    if ([(NSString*)key isEqualToString:@"http_method"]) {
        ...
        @try {
            if (self.payloadClassName) {
                if (![NSClassFromString(self.payloadClassName) isSubclassOfClass:[KGObject class]]) {
                    [CNDebugLog message:@"%@ can't re-construct from archived data.",NSStringFromClass([self.payLoad class])];
                    return;
                }
                
                KGObject *obj = [[NSClassFromString(self.payloadClassName) alloc] init];
...
- (void)setPayLoad:(id<KGObjectProtocol>)payLoad{
    
    _payLoad = payLoad;
    if (_payLoad && [_payLoad isKindOfClass:[KGObject class]]) {
